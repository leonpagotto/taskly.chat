...
- Exports mergeCandidates(list) applying Levenshtein <=2 OR Jaccard >=0.85 similarity threshold.
- Retains highest confidence among merged items; collects variants array.
- Ignores stop words (documented list) during token similarity computation.
- Processes 20 candidate strings under 5ms average in local benchmark.
- Unit tests cover: merge, no merge distinct, performance budget check.
