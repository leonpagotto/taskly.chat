...
function detectIntent(text, notes) {
    ...
    // fallback heuristic
    if (/\b(remind|reminder)\b/i.test(text))
        return 'create_reminder';
    if (/\b(task|todo|to-do)\b/i.test(text))
        return 'create_task';
...
function extractEntities(intent, text, notes) {
    const missing = [];
    ...
    if (intent === 'create_task') {
        ...
        if (p) {
            ...
            entities.priority = raw.includes('urgent') ? 'high' : raw.replace('priority ', '');
...
function normalizeTimeToken(token) {
    ...
    if (/^\d{1,2}:\d{2}(am|pm)?$/.test(lower)) {
        ...
        const today = new Date();
        let [time, ap] = lower.match(/^(\d{1,2}:\d{2})(am|pm)?$/).slice(1);
