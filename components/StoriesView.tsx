												...
												.map(s => {
													...
													return (
														<button
															...
															className="w-full text-left bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all cursor-grab active:cursor-grabbing select-none min-h-[100px]"
														>
															<div className="flex flex-col gap-2">
																{/* Title and Icon */}
																<div className="flex items-center justify-between gap-2">
																	<div className="flex items-center gap-2 flex-1 min-w-0">
																		<div className="w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${(category?.color || project?.color || '#64748B')}20` }}>
																			<Icon name={category?.icon || project?.icon || 'auto_stories'} style={{ color: category?.color || project?.color || '#64748B' }} className="text-sm" />
																		</div>
																		<div className="font-semibold text-sm text-gray-900 dark:text-white line-clamp-2 leading-snug">{s.title}</div>
																	</div>
																</div>
																
																{/* Story Points & Assignee at bottom */}
																<div className="flex items-center justify-between gap-2 mt-auto">
																	{estimateLabel && (
																		<span className="px-2 py-1 rounded-md bg-gradient-to-r from-[var(--color-primary-600)] to-[var(--color-primary-end)] text-white text-xs font-semibold whitespace-nowrap">{estimateLabel}</span>
																	)}
																	{s.assigneeName && (
																		<span className="px-2 py-1 rounded-md bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-semibold whitespace-nowrap truncate max-w-[120px]" title={s.assigneeName}>{s.assigneeName}</span>
																	)}
																</div>
															</div>
														</button>
