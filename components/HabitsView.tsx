...
const HabitCard: React.FC<{
  ...
  onToggleCompletion: (date: string) => void;
    ...
    const calculateStreak = () => {
        ...
        for (let i = startDayOffset; i < 365 * 5; i++) { // Check up to 5 years
            const date = new Date();
            date.setDate(date.getDate() - i);
            if (date < new Date(habit.recurrence.startDate)) break;

            if (isHabitDueOnDate(habit, date)) {
                if (habit.completionHistory.includes(getISODate(date))) {
                    streak++;
                } else {
                    break; 
                }
    ...
    const calculateCompletionRate = () => {
        ...
        if (totalDueDays === 0) return 100; // If created today and due today, total is 1, so this won't be hit. If not due, 0 is okay.
    ...
    <div className="bg-white dark:bg-gray-700 p-3 rounded-xl flex flex-col gap-0.5 group transition-all hover:shadow-md">
             <div className="flex items-start justify-between gap-3">
                {category ? (
                    ...
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 mt-1 bg-gray-200 dark:bg-gray-600">
                ...
                <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-2 -mr-2 -mt-1 text-gray-500 hover:text-blue-400 rounded-full transition-colors opacity-0 group-hover:opacity-100">
                    <EditIcon />
                </button>
            </div>
            ...
            <div className="flex justify-between items-center">
                {lastSevenDays.map(day => {
                    ...
                    if (isCompleted) {
                        dayStyle = 'bg-yellow-400 text-gray-900';
                    } else if (isDue) {
                        dayStyle = 'hover:bg-gray-300 dark:hover:bg-gray-500';
                    }
                    
                    let borderStyle = 'border-2 border-transparent';
                    if (isDue && !isCompleted) {
                        borderStyle = `border-2 ${isToday ? 'border-[var(--color-primary-600)]' : 'border-[var(--color-primary-600)]/50'}`;
                    }
...
const HabitsView: React.FC<HabitsViewProps> = (props) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-100 dark:bg-gray-800 h-full">
            <Header title={t('habits')} onToggleSidebar={onToggleSidebar}>
        <button onClick={onNewHabitRequest} className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[var(--color-primary-600)] to-purple-600 text-white rounded-full font-semibold hover:shadow-lg transition-all text-sm">
            <NewHabitIcon />
            ...
            <div className="text-center text-gray-500 flex flex-col items-center justify-center h-full p-6">
                ...
                <p className="max-w-md mt-1 mb-6">{habits.length > 0 ? 'No habits match the current filters.' : t('no_habits_yet_subtitle')}</p>
                 <button onClick={onNewHabitRequest} className="mt-6 px-6 py-3 bg-gradient-to-r from-[var(--color-primary-600)] to-purple-600 text-white rounded-full font-semibold hover:shadow-lg transition-all">
                    {t('create_habit')}
                </button>
            </div>
        )}
